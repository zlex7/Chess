...
public class TextChess {
	...
	public void move(String m){
		...
		//while(mwhile){}
		m=m.toLowerCase();
		if(m.equals("takeback")){
			int numMoves=10000;
			System.out.println("Please specify the number of moves you wish to takeback");
			if(scan.hasNextInt()){
			numMoves = scan.nextInt();
			}
			while(numMoves>moves.size()/5){
			System.out.println("I'm sorry, it's not possible to take that many moves. The number of moves you've made this game is " + moves.size()/5 );
			numMoves=scan.nextInt();
			}
			takeback(numMoves);
		}
			...
			m=m.toLowerCase();
			Pattern isValid = Pattern.compile("[a-z][a-z][1-8]");
			Matcher match = isValid.matcher(m);
			if(match.matches()){
			char firstindex = m.charAt(0);
			char secondindex = m.charAt(1);
			char thirdindex=m.charAt(2);
		for(char row1[] : board){
			for(char s: row1){
				System.out.println("2nd for statement");
				System.out.println(firstindex);
				System.out.println(s);
				
				if(firstindex==Character.toLowerCase(firstindex)){
					System.out.println("first if statement");
					if(color){
						System.out.println("second if statement");
						if(s==Character.toUpperCase(firstindex)){
							System.out.println("third if statement");
							row=Integer.parseInt(m.substring(2,3))-1;
							col=getfile(m.charAt(1));
							if(checkmove(r,c,row,col,m,false,false)){
								System.out.println("-----------");
								System.out.println("checkmove returns true");
								...
								System.out.println("----------");
								checkmove(r,c,row,col,m,false,true);
								System.out.println("checkmove for white");
								temp=board[row][col];
								...
								if(!checkforcheck('w',0,0,0,0)){
									oldrow=r;
									oldcol=c;
									System.out.println("new row:" + Integer.toString(row));
									System.out.println(Integer.toString(oldrow));
									System.out.println(board[r][c]);
									System.out.println("new col:" +Integer.toString(col));
									System.out.println(Integer.toString(oldcol));
									move=true;
									//firstpiecerow=r;
									//firstpiececol=c;
									//if(count==1){
						...
						if(s==Character.toLowerCase(firstindex)){
							row=Integer.parseInt(m.substring(2,3))-1;
							col=getfile(m.charAt(1));
							if(checkmove(r,c,row,col,m,false,false)){
								checkmove(r,c,row,col,m,false,true);
		...
		if(move){
			if((row==oldrow+1 || row==oldrow-1) && (col==oldcol-1 || col==oldcol+1) && (board[row][col]=='0')){
	...
	public static boolean checkmove(int oldr,int oldc,int r,int c,String move,boolean bool_cforc,boolean real_move){
		...
		piece=Character.toLowerCase(move.charAt(0));
		switch(piece){
		case 'p':
					...
					for(int k=0;k<board.length;k++){
						for(int l=0;l<board[k].length;l++){
							if(board[i][j]=='B' && board[k][l]=='k'){
								System.out.println("DETECTED BISHOP:");
								System.out.println(i);
								System.out.println(j);
								System.out.println(board[k][l]);
								System.out.println(k);
								System.out.println(l);
								System.out.println(String.valueOf(checkmove(i,j,k,l,Character.toString(board[i][j]),true,false)));
								System.out.println("------------");
							}
							if(checkmove(i,j,k,l,Character.toString(board[i][j]),true,false)){
								if(board[k][l]=='K'){
